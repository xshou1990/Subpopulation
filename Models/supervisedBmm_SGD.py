...
class supervisedBmm_SGD(object):
...
#        self.Beta = 0 # Gaussian diagnoval covariances: beta^2
    ...
    def fit(self, data, targetCol, cadreFts=None, predictFts=None, dataVa=None, inits=None, 
        ...
        if inits is not None and 'Mu' in inits:
            ...
            Mu = tf.Variable(np.random.beta(a = 2, b= 2, size=(Pcadre,self.M)), 
...
###################### Membership ########################################   
        ...
        mvb = tfp.distributions.Bernoulli(probs= tf.tile(tf.transpose(Mu),[tf.shape(Xcadre)[0],1]))
        
        # log B(x|mu_k)
        LogB_ik = tf.reshape(tf.reduce_sum(mvb.log_prob(rep_Xcadre),axis=1),(tf.shape(Xcadre)[0],self.M), name = 'LogB_ik') 
        
        # max log B(x|mu_k), column-wise reduce max
        MaxlogB_ik = tf.expand_dims(tf.reduce_max(LogB_ik, axis =1),axis=1, name ='MaxlogB_ik')
        
        # log B(x|mu_k) - max log B(x|mu_k)
        Norm_LogB_ik = tf.subtract(LogB_ik, tf.tile(MaxlogB_ik, 
                                                tf.constant([1,LogB_ik.get_shape()[1].value])),  name = 'Norm_LogB_ik') 
        # B(x|mu_k) 
        B_ik = tf.exp( Norm_LogB_ik , name = 'B_ik')
        
        # f(x,z) = pi * B_ik
        joint_pdf = tf.multiply(pi, B_ik, name = 'joint_pdf')  
        # f(z|x) --> normalized pmf,posterior
...
###################### Objective: NLL ########################################  
        ...
        loss = tf.divide( - tf.reduce_sum(tf.multiply(pi, tf.add(LogB_ik, Loglogiprob))) , tf.dtypes.cast(tf.shape(Xcadre)[0], dtype = tf.float32), name ='loss')
        loss_opt = loss + l2_W
        loss_full = loss_opt + l1_W
        optimizer = tf.compat.v1.train.AdamOptimizer(learning_rate=eta).minimize(loss_opt)
        
        ## projection Mu to box (0,1)
        proj_Mu = tf.compat.v1.assign(Mu, tf.minimum( tf.maximum(Mu, 0.000001),0.99999) )
        ...
        with tf.compat.v1.Session() as sess:
            ...
            self.Mu, self.gamma, self.W, self.w0 = Mu.eval(), gamma.eval(), W.eval(), w0.eval()
